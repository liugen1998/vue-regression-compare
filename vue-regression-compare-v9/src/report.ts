...
import type { CoverageRow, PageConfig, ResultRow, ResultStatus, RunContext, Severity } from './types.js';
import { enrichRows } from './diagnosis.js';
import { ensureDir, escapeHtml } from './utils.js';
import { coverageFromConfigAndRows, estimatePlan } from './plan.js';

export async function writeReports(inputRows: ResultRow[], outputDir: string, configs: PageConfig[] = [], ctx?: RunContext): Promise<void> {
  await ensureDir(outputDir);
  const rows = enrichRows(inputRows);
  const coverage = coverageFromConfigAndRows(configs, rows as any) as CoverageRow[];
  const plans = configs.map(c => estimatePlan(c, ctx?.mode ?? 'default'));
  await Promise.all([
    ...
    fs.writeFile(path.join(outputDir, 'plan-preview.json'), JSON.stringify(plans, null, 2), 'utf8'),
    writeHtml(rows, coverage, plans, path.join(outputDir, 'report.html')),
    writeExcel(rows, coverage, plans, path.join(outputDir, 'report.xlsx'))
  ]);
}

async function writeHtml(rows: ResultRow[], coverage: CoverageRow[], plans: any[], file: string): Promise<void> {
  const summary = summarize(rows, coverage);
...
<style>
body{font-family:Arial,"Microsoft YaHei",sans-serif;margin:24px;color:#1f2328;background:#fff}h1{margin:0 0 8px}h2{margin-top:28px}.meta,.sub{color:#6a737d;font-size:12px}.business-summary{margin:16px 0;padding:14px 16px;background:#f6f8fa;border:1px solid #d0d7de;border-radius:10px;line-height:1.8}.business-summary b{color:#0a3069}.summary{display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:10px;margin:18px 0}.card{border:1px solid #d0d7de;border-radius:10px;padding:12px;background:#fff;cursor:pointer}.card:hover{background:#f6f8fa}.card .label{color:#57606a;font-size:13px}.card .num{font-weight:700;font-size:24px;margin-top:4px}.toolbar{display:flex;flex-wrap:wrap;gap:12px;align-items:flex-end;padding:12px;border:1px solid #d0d7de;border-radius:10px;background:#fff}.toolbar label{display:block;font-size:12px;color:#57606a;margin-bottom:4px}.toolbar select,.toolbar input{height:32px;min-width:150px;border:1px solid #d0d7de;border-radius:6px;padding:0 8px}.toolbar input{min-width:320px}.toolbar button{height:32px;border:1px solid #0969da;background:#0969da;color:#fff;border-radius:6px;padding:0 10px;cursor:pointer}.countbar{margin:10px 0;color:#57606a}.table-wrap{overflow:auto;border:1px solid #d0d7de;border-radius:8px}table{border-collapse:collapse;width:100%;min-width:2400px}th,td{border-bottom:1px solid #d8dee4;border-right:1px solid #d8dee4;padding:8px;vertical-align:top;font-size:13px}.table-wrap thead,.table-wrap thead tr,.table-wrap thead th{position:static!important;top:auto!important;z-index:auto!important}th{background:#f6f8fa;text-align:left;white-space:nowrap}tr:hover{background:#fffef0}code{white-space:pre-wrap;font-family:Consolas,monospace;font-size:12px}.value-cell{max-width:360px;white-space:pre-wrap;word-break:break-word}.badge,.sev{display:inline-block;padding:2px 8px;border-radius:999px;font-weight:700;white-space:nowrap}.badge-pass{background:#dafbe1;color:#116329}.badge-fail{background:#ffebe9;color:#a40e26}.badge-perf{background:#fff1c2;color:#7d4e00}.badge-error{background:#ffebe9;color:#a40e26}.badge-skip{background:#eaeef2;color:#57606a}.sev-s0{background:#a40e26;color:#fff}.sev-s1{background:#ffebe9;color:#a40e26}.sev-s2{background:#fff1c2;color:#7d4e00}.sev-s3{background:#eaeef2;color:#57606a}.diff-old{background:#ffd6d6;color:#a40000;font-weight:700;border:1px solid #ff8a8a;border-radius:3px;padding:0 2px}.diff-new{background:#d6ecff;color:#004a99;font-weight:700;border:1px solid #8ac5ff;border-radius:3px;padding:0 2px}.thumb{display:inline-block;margin-top:4px;max-width:96px;max-height:72px;border:1px solid #ccc;border-radius:4px;vertical-align:top;cursor:zoom-in}.modal{display:none;position:fixed;inset:0;background:rgba(0,0,0,.72);z-index:99;align-items:center;justify-content:center}.modal.open{display:flex}.modal img{max-width:94vw;max-height:90vh;background:#fff;box-shadow:0 12px 40px rgba(0,0,0,.35)}.modal .close{position:fixed;top:20px;right:28px;color:#fff;font-size:28px;cursor:pointer}.coverage-table{min-width:1100px}.plan-table{min-width:1200px}
</style></head><body>
<h1>Vue2/Vue3 升级验收报告</h1><div class="meta">生成时间：${escapeHtml(new Date().toLocaleString())}</div>
<div class="business-summary"><div><b>验收口径：</b>展示值必须完全一致；Vue3 耗时高于 Vue2 即性能下降；交互功能默认只做确定性匹配，不做模糊猜测。</div><div><b>覆盖口径：</b>报告不仅展示差异，也展示已覆盖、未覆盖和需人工补齐项，防止“跑了但漏测”。</div></div>
<div class="summary">${summaryCards(summary)}</div>
<h2>测试策略预览</h2><div class="table-wrap"><table class="plan-table"><thead><tr><th>页面</th><th>策略</th><th>预计场景</th><th>指标检查</th><th>交互检查</th><th>性能检查</th><th>筛选策略</th><th>页签</th><th>警告</th></tr></thead><tbody>${plans.map(p => `<tr><td>${escapeHtml(p.pageName)}</td><td>${escapeHtml(p.strategy)}</td><td>${p.scenarioCount}</td><td>${p.metricChecks}</td><td>${p.interactionChecks}</td><td>${p.performanceChecks}</td><td>${escapeHtml(p.filterScenarios.map((f:any)=>`${f.filterName}:${f.strategy}(${f.options.length})`).join('；')||'-')}</td><td>${escapeHtml(p.tabScenarios.join('，')||'-')}</td><td>${escapeHtml(p.warnings.join('；')||'-')}</td></tr>`).join('')}</tbody></table></div>
<h2>覆盖情况</h2><div class="table-wrap"><table class="coverage-table"><thead><tr><th>模块</th><th>发现</th><th>计划</th><th>已验证</th><th>忽略</th><th>无法判断</th><th>覆盖率</th><th>缺口</th><th>建议</th></tr></thead><tbody>${coverage.map(c => `<tr><td>${escapeHtml(c.module)}</td><td>${c.discovered}</td><td>${c.planned}</td><td>${c.verified}</td><td>${c.ignored}</td><td>${c.unknown}</td><td>${escapeHtml(c.coverageRate)}</td><td>${escapeHtml(c.gap)}</td><td>${escapeHtml(c.suggestion)}</td></tr>`).join('')}</tbody></table></div>
...
<div class="table-wrap"><table id="detailTable"><thead><tr><th>#</th><th>页面</th><th>场景</th><th>页签</th><th>筛选器</th><th>筛选值</th><th>类型</th><th>验证项</th><th>级别</th><th>差异归因</th><th>Vue2 值</th><th>Vue3 值</th><th>结果</th><th>异常类型</th><th>说明</th><th>差异说明</th><th>建议处理动作</th><th>selector</th><th>Vue2耗时ms</th><th>Vue3耗时ms</th><th>性能变化</th><th>Vue2截图</th><th>Vue3截图</th></tr></thead><tbody>${detailRows}</tbody></table></div><div id="imgModal" class="modal"><span class="close">×</span><img alt="截图"/></div>
<script>
const rows=[...document.querySelectorAll('#detailTable tbody tr')];const pageFilter=document.getElementById('pageFilter'),categoryFilter=document.getElementById('categoryFilter'),statusFilter=document.getElementById('statusFilter'),severityFilter=document.getElementById('severityFilter'),attributionFilter=document.getElementById('attributionFilter'),searchBox=document.getElementById('searchBox'),visibleCount=document.getElementById('visibleCount');
...
async function writeExcel(rows: ResultRow[], coverage: CoverageRow[], plans: any[], file: string): Promise<void> {
  ...
  const coverageSheet = workbook.addWorksheet('覆盖情况');
  coverageSheet.columns = [
    { header: '模块', key: 'module', width: 36 }, { header: '发现', key: 'discovered', width: 10 }, { header: '计划', key: 'planned', width: 10 }, { header: '已验证', key: 'verified', width: 10 }, { header: '忽略', key: 'ignored', width: 10 }, { header: '无法判断', key: 'unknown', width: 12 }, { header: '覆盖率', key: 'coverageRate', width: 12 }, { header: '缺口', key: 'gap', width: 36 }, { header: '建议', key: 'suggestion', width: 60 }
  ];
  coverage.forEach(c => coverageSheet.addRow(c));
  formatGenericSheet(coverageSheet);

  const planSheet = workbook.addWorksheet('策略预览');
  planSheet.columns = [
    { header: '页面', key: 'pageName', width: 24 }, { header: '策略', key: 'strategy', width: 14 }, { header: '预计场景', key: 'scenarioCount', width: 12 }, { header: '指标检查', key: 'metricChecks', width: 12 }, { header: '交互检查', key: 'interactionChecks', width: 12 }, { header: '性能检查', key: 'performanceChecks', width: 12 }, { header: '筛选策略', key: 'filterSummary', width: 70 }, { header: '页签', key: 'tabSummary', width: 40 }, { header: '警告', key: 'warningSummary', width: 70 }
  ];
  plans.forEach(p => planSheet.addRow({ ...p, filterSummary: p.filterScenarios.map((f:any)=>`${f.filterName}:${f.strategy}(${f.options.length})`).join('；') || '-', tabSummary: p.tabScenarios.join('，') || '-', warningSummary: p.warnings.join('；') || '-' }));
  formatGenericSheet(planSheet);

  const summarySheet = workbook.addWorksheet('总览');
  summarySheet.columns = [{ header: '类型', key: 'type', width: 30 }, { header: '数量', key: 'count', width: 12 }];
  Object.entries(summarize(rows, coverage)).forEach(([type, count]) => summarySheet.addRow({ type, count }));
...
function summarize(rows: ResultRow[], coverage: CoverageRow[]): Record<string, number> {
  const pages = new Set(rows.map(r => r.pageKey)).size;
  return { '页面数': pages, '总项数': rows.length, '通过': rows.filter(r => r.status === '通过').length, '问题项': rows.filter(isProblem).length, 'S0阻断': rows.filter(r => r.severity === 'S0阻断').length, 'S1严重': rows.filter(r => r.severity === 'S1严重').length, 'S2一般': rows.filter(r => r.severity === 'S2一般').length, '不通过': rows.filter(r => r.status === '不通过').length, '需人工确认': rows.filter(r => r.status === '需人工确认').length, '性能下降': rows.filter(r => r.status === '性能下降').length, '执行异常': rows.filter(r => r.status === '执行异常').length, '覆盖缺口': coverage.filter(c => c.gap !== '-').length, 'Vue3缺失': rows.filter(r => r.attribution === 'Vue3缺失').length, 'Vue3多出': rows.filter(r => r.attribution === 'Vue3多出').length, '展示值差异': rows.filter(r => r.attribution === '展示值差异').length, 'selector配置问题': rows.filter(r => r.attribution === 'selector配置问题').length, '指标描述一致失败': rows.filter(r => r.category === '指标描述一致' && isProblem(r)).length, '页面交互功能一致失败': rows.filter(r => r.category === '页面交互功能一致' && isProblem(r)).length, '性能不下降失败': rows.filter(r => r.category === '性能不下降' && r.status === '性能下降').length };
}

function summaryCards(summary: Record<string, number>): string {
  const cardAttrs: Record<string, string> = { '通过': 'data-status="通过"', '不通过': 'data-status="不通过"', '需人工确认': 'data-status="需人工确认"', '性能下降': 'data-status="性能下降"', '执行异常': 'data-status="执行异常"', 'S0阻断': 'data-severity="S0阻断"', 'S1严重': 'data-severity="S1严重"', 'S2一般': 'data-severity="S2一般"', 'Vue3缺失': 'data-attribution="Vue3缺失"', 'Vue3多出': 'data-attribution="Vue3多出"', '展示值差异': 'data-attribution="展示值差异"', 'selector配置问题': 'data-attribution="selector配置问题"', '指标描述一致失败': 'data-category="指标描述一致"', '页面交互功能一致失败': 'data-category="页面交互功能一致"', '性能不下降失败': 'data-category="性能不下降" data-status="性能下降"' };
